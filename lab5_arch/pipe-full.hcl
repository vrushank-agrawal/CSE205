...
# 1) In the fetch stage, we need the iaddq function to exist in 
#    icode so we add it to the bool instr_valid. iaddq also needs
#	 regids and valC.
# 2) In the decode stage, the iaddq function needs the D_rB register
#	 hence, we add it to the srcB word.
# 3) In the execution stage, the ALU needs both valC and valB 
#	 hence we add iaddq to the words aluA and aluB respectively.
#	 We also need to update the condition codes in bool set_cc hence
#	 we update the word by adding an or condition for E_icode. 
# 4) In the memory stage, nothing happens so we do not change it.
# 5) For the write back stage, we define the adress for the destination 	
#	 of the value computed as D_rB which is set in the word d_dstE.
# 6) For PC Update, we use the default value hence nothing changes.
